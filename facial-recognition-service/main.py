...
# Environment variables (put in .env, loaded below):
...
#     # asyncpg accepts the string 'require' which wraps the default SSL context.
...
# # Adjust `allow_origins` to your production URL before deploying.
# app.add_middleware(
#     CORSMiddleware,
...
#     so the service can still function during development without Redis.
#     """
#     # ── Graceful degradation: skip if Redis is not running ────────────────────
#     if redis is None:
#         logger.debug("Rate limiter skipped – Redis not connected.")
#         return

#     # Get the real client IP (honours X-Forwarded-For if behind a proxy)
#     client_ip: str = request.client.host if request.client else "unknown"

#     # Calculate which window slot we're in (integer division rounds down)
#     import time
#     window_slot = int(time.time()) // RATE_LIMIT_WINDOW_SECONDS

#     # Build unique key for this IP + window
#     redis_key = f"rate_limit:{client_ip}:{window_slot}"

#     # Atomically increment the counter
#     current_count: int = await redis.incr(redis_key)

#     # On the FIRST increment, set the expiry so the key is auto-deleted
#     if current_count == 1:
...
#         # Attach Retry-After header so well-behaved clients can back off
#         raise HTTPException(
...
# # We use `dependencies` on the app-level router instead of per-route:
# # (FastAPI supports this via the `dependencies` list on the FastAPI constructor,
# #  but we also add it to individual routes for explicitness.)
...
#     The Next.js admin UI sends this when a student profile photo is uploaded.
#     """
#     studentId: int         # maps to StudentProfile.id in the Prisma schema
#     photoUrl: HttpUrl      # publicly accessible URL of the student's photo
...
#     """
#     Downloads an image from `url` asynchronously using httpx and returns
#     the raw bytes.  All I/O happens in memory – nothing is written to disk.

#     Raises:
#         HTTPException 502  – if the remote server is unreachable or returns
#                              a non-2xx response.
#     """
#     async with httpx.AsyncClient(follow_redirects=True, timeout=15.0) as client:
#         try:
#             response = await client.get(url)
#             response.raise_for_status()  # throws httpx.HTTPStatusError on 4xx/5xx
...
#     Processing pipeline (all in RAM):
#       1. Wrap bytes in an in-memory BytesIO buffer.
#       2. face_recognition.load_image_file() reads it (internally uses PIL).
#          face_recognition always loads images as RGB arrays.
#       3. face_recognition.face_encodings() runs dlib's HOG face detector
#          and the deep-metric embedding model, returning one 128-D float64
#          numpy array per detected face.

#     Raises:
#         HTTPException 422  – if 0 or more than 1 face is detected.
#     """
#     # BytesIO allows face_recognition to treat bytes as a file without disk I/O
#     image_buffer = io.BytesIO(image_bytes)

#     # Load image as RGB numpy array (shape: H × W × 3)
#     rgb_image: np.ndarray = face_recognition.load_image_file(image_buffer)

#     # Detect faces and compute 128-D embeddings
#     # num_jitters=1 is fast; increase to 10 for higher accuracy at the cost of speed
...
#                 "Please ensure the photo is clear and well-lit."
#             ),
#         )
...
#             status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
#             detail=(
#                 f"{len(encodings)} faces detected in the image. "
#                 "The student profile photo must contain exactly one face."
#             ),
#         )

#     # Convert numpy float64 array to a plain Python list for JSON serialisation
...
#     dependencies=[Depends(rate_limit)],  # ← rate limit applied here explicitly too
# )
...
# ) -> dict[str, Any]:
#     """
#     Endpoint called by the Next.js admin panel when a teacher/admin uploads
#     a profile photo for a student.

#     Full request lifecycle
#     ──────────────────────
#     1. Validate the request body (Pydantic handles this automatically).
#     2. Download the image from `photoUrl` into memory using httpx.
#     3. Run dlib face detection + deep-metric encoding via face_recognition.
#     4. Persist the 128-D embedding as a pgvector `vector(128)` column
#        in the `StudentProfile` table using a raw SQL UPDATE via asyncpg.
#     5. Return a success JSON response.

#     The embedding is stored as a pgvector literal string in the format:
#       '[0.123, -0.456, 0.789, ...]'
#     which PostgreSQL+pgvector interprets natively as a vector(128).
#     """
#     logger.info("Processing photo for studentId=%d url=%s", body.studentId, body.photoUrl)

#     # ── Step 1: Download image bytes (network I/O) ────────────────────────────
#     image_bytes: bytes = await _download_image_bytes(str(body.photoUrl))

#     # ── Step 2: Extract face embedding (CPU-intensive – runs in thread pool) ──
#     # face_recognition is synchronous (not async-native).  Running it directly
#     # in an async handler would block the event loop for the entire inference
#     # duration (~50–200ms).  We offload it to the default thread pool executor.
...
#     # ── Step 3: Format embedding as a pgvector literal string ─────────────────
#     # pgvector expects:  '[dim1, dim2, ..., dim128]'
#     # numpy tolist() gives us a Python list of plain floats, which we join:
#     vector_literal: str = "[" + ",".join(str(v) for v in embedding) + "]"

#     # ── Step 4: Persist to PostgreSQL via asyncpg ─────────────────────────────
#     # We use a raw parameterised UPDATE query.
#     # The CAST(... AS vector) is necessary because asyncpg does not know the
#     # pgvector type natively – it would otherwise try to send it as plain text
#     # which Postgres might refuse.  The explicit cast ensures type safety.
#     #
#     # Schema alignment:
#     #   Table:   "StudentProfile"   (Prisma uses PascalCase with quoted identifiers)
#     #   Column:  "faceEmbedding"    (the Unsupported("vector(128)") column)
...
#     # asyncpg returns a status tag like "UPDATE 1" – we parse it to confirm
...
# ) -> dict[str, Any]:
#     """
#     Endpoint called by the Teacher UI when a teacher uploads a classroom photo.

#     Full request lifecycle
#     ──────────────────────
#     1. Read the uploaded file bytes into memory (no temp files).
#     2. Detect ALL faces in the group photo → N embeddings.
#     3. Fetch every StudentProfile that has a stored faceEmbedding from the DB.
#     4. For each detected face, find the closest stored embedding using cosine
#        distance.  If distance < VECTOR_DISTANCE_THRESHOLD (0.4) → match.
#     5. Return the list of matched StudentProfile IDs (de-duplicated).

#     pgvector cosine distance SQL
#     ────────────────────────────
#     The `<=>` operator computes cosine distance between two vector(128) values.
#     Cosine distance = 1 − cosine_similarity, so:
#         distance 0.0 → identical vectors (same face)
#         distance 1.0 → perfectly orthogonal (completely different face)
#         distance 2.0 → opposite vectors (impossible for face embeddings in practice)

#     We retrieve ALL stored embeddings in a single query and do the comparison
#     in Python (numpy).  This is fine for a college with < 5,000 students.
#     For larger datasets, switch to the pgvector ANN index approach:
...
#     # ── Step 2: Detect all faces in the group photo (offload to thread pool) ──
#     loop = asyncio.get_running_loop()
...
#     # ── Step 3: Fetch all stored embeddings from the database ─────────────────
#     # We select only students who already have a faceEmbedding stored
#     # (NULL embeddings are students who haven't been registered yet).
#     #
#     # The ::text cast is used to retrieve the vector as a plain string.
#     # We then parse it back to a numpy array in Python.
#     # (asyncpg doesn't have a built-in pgvector codec, so text is safest.)
#     fetch_sql = """
...
#     # ── Step 4: Parse DB embeddings back into numpy arrays ────────────────────
#     # pgvector returns the vector as a string:  '[0.12,-0.34,0.56,...]'
#     # We strip the brackets and split on commas to reconstruct the numpy array.
...
#         # Strip surrounding brackets then parse floats
...
#     logger.info("Loaded %d stored student embeddings.", len(db_student_ids))

#     # ── Step 5: Match each detected face against all stored embeddings ─────────
#     # face_recognition.face_distance() computes the Euclidean distance between
#     # one "unknown" encoding and an array of "known" encodings.
#     # This aligns with pgvector cosine distance for normalised vectors
#     # (face_recognition embeddings are L2-normalised by design, so
#     # cosine distance ≈ 0.5 * euclidean_distance² for unit vectors).
#     # We use the same 0.4 threshold specified in the architecture doc.
#     matched_student_ids: set[int] = set()

#     known_encodings_array = np.array(db_embeddings)  # shape: (N_students, 128)

#     for face_encoding in group_encodings:
#         # Compute euclidean distance from this face to every stored embedding.
...
#         # Compare the converted Cosine distance against your 0.4 threshold
...
#     """
#     Synchronous helper (runs in thread pool) that:
#       1. Decodes image bytes → RGB numpy array.
#       2. Detects all face bounding boxes (HOG model – fast).
#       3. Computes a 128-D embedding for each detected face.

#     Returns a list of numpy arrays, one per detected face.
#     If no faces are found, returns an empty list (not an error).
#     """
#     image_buffer = io.BytesIO(image_bytes)
#     rgb_image: np.ndarray = face_recognition.load_image_file(image_buffer)

#     # face_locations uses the HOG model by default (model="hog").
#     # Switch to model="cnn" for higher accuracy (requires a GPU for speed).
...
# # ─────────────────────────────────────────────────────────────────────────────
# # Entry point  (run directly with `python main.py` during development)
# # ─────────────────────────────────────────────────────────────────────────────
