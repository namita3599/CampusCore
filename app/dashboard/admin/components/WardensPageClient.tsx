...
export default function WardensPageClient({
  wardens,
  hostels,
}: {
  wardens: Warden[];
  ...
  const filtered = useMemo(() => {
    ...
    if (filterAssigned === "yes") list = list.filter((w) => w.hostels.length > 0);
    if (filterAssigned === "no") list = list.filter((w) => w.hostels.length === 0);
    if (filterHostel)
      list = list.filter((w) => w.hostels.some((h) => h.id === parseInt(filterHostel)));
    ...
    return list;
  }, [wardens, search, sortDir, filterHostel, filterAssigned]);

  const activeFilterCount = [filterHostel, filterAssigned].filter(Boolean).length;
    ...
    <div className="space-y-5">
        ...
        <div className="bg-zinc-50/80 border border-zinc-200 rounded-2xl p-4 shadow-sm">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-md">
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">Hostel Assigned</label>
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">Specific Hostel</label>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-sm font-medium text-zinc-600">
          {filtered.length === wardens.length
            ? <>{wardens.length} <span className="text-zinc-400">wardens</span></>
            : <><span className="font-bold text-zinc-950">{filtered.length}</span> <span className="text-zinc-400">of {wardens.length} wardens</span></>
          }
        </span>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-zinc-200 bg-white p-16 text-center shadow-sm">
          ...
          <p className="text-zinc-500 text-sm font-medium">No wardens match your search or filters.</p>
          <button onClick={clearAll} className="mt-3 text-xs text-zinc-600 underline hover:text-zinc-900">Clear all filters</button>
        </div>
      ) : (
        <WardensTable wardens={filtered as any} hostels={hostels} />
      )}
    </div>
  );
}
